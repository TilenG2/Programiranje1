...
def v_pot(s): #s = ">100 ^42 <13"
    vrnitev = []
    coords = s.split()
    for coord in coords:
        vrnitev.append((coord[:1], int(coord[1:])))
    return vrnitev

def odsek(x, y, smer, razdalja): #(x, y, "v", 3)
    odseki = []
    multx = 0
    multy = 0
    if smer == "^": multy = 1
    ...
    for i in range(0, razdalja + 1):
        odseki.append((x + i * multx, y + i * multy))
    return odseki
...
def tocke(s): #s = ("v", 3)
    ...
    for (smer, razdalja) in s:
        tockexy += odsek(x, y, smer, razdalja)
        x, y = tockexy.pop()
    tockexy.append((x, y))
    return tockexy

def presecisca(s, t): #s = ">100 ^42 <13", t = ">100 ^42 <13"
    return sorted(list(set(tocke(v_pot(s))) & set(tocke(v_pot(t)))))

def prvo_srecanje(s, t): #s = ">100 ^42 <13", t = ">100 ^42 <13"
    pot1 = tocke(v_pot(s))
    pot2 = tocke(v_pot(t))
    i = 1
    while True:
