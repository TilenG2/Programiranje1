...
def najboljsa_povezava(zemljevid):
    return max([(tocke, pot) for pot, tocke in [[pot, vrednost_povezave(pot, zemljevid)] for pot, _ in zemljevid.items()]])[1]

def vrednost_poti(pot, zemljevid):
    return sum(vrednost_povezave(povezava, zemljevid) for povezava in zip(pot, pot[1:]))
...
def najbolj_uporabna(pot, zemljevid):
    ...
    for pot in zip(pot, pot[1:]):
        for vescina in zemljevid[pot]:
            pojavitveVescin[vescina] += 1

    maxcountVescine = max(pojavitveVescin.values())
    if maxcountVescine != 0:
        for vescina in pojavitveVescin:
            if pojavitveVescin[vescina] == maxcountVescine:
                return vescina
    return None
    
def mozna_pot(pot, zemljevid, vescine):
    for kords in zip(pot, pot[1:]):
        if kords not in zemljevid: return False

    for kords in zip(pot, pot[1:]):
        if vescine | zemljevid[kords] != vescine: return False
        vescine = vescine - zemljevid[kords]
    return True

def koncna_tocka(pot, zemljevid, vescine):
    for kords in zip(pot, pot[1:]):
        if kords not in zemljevid: return kords[0]
    vescine = dict(vescine)
    for kords in zip(pot, pot[1:]):
        for vescina in zemljevid[kords]:
            if vescina not in vescine: break
            vescine[vescina] -= 1
            if vescine[vescina] < 0: break
        else:
            continue
        return kords[0]
    return pot[-1:]
...
def do_nagrade(pot, zemljevid, meja):
    ...
    sestevek = 0
    for kords in zip(pot, pot[1:]):
        sestevek += vrednost_povezave(kords, zemljevid)
        if sestevek > meja:
            return kords[0]
    return pot[-1:]

def naslednje_tocke(tocke, zemljevid, vescine):
    return {x1 for x1, x2 in zemljevid if (x1 in tocke or x2 in tocke) and zemljevid[(x1, x2)] <= vescine}

def dosegljive(tocka, zemljevid, vescine):
    tocke = naslednje_tocke(tocka, zemljevid, vescine)
    while True:
        if naslednje_tocke(tocke, zemljevid, vescine) == tocke: return tocke
        else: tocke = naslednje_tocke(tocke, zemljevid, vescine)

def dosegljive_n(tocka, zemljevid, meja):
    def doseg(tockameja, zemljevid, banned):
        tocka, meja = tockameja
        dosegljivetocke = set(tocka)

        for x1, x2 in ((x1, x2) for x1, x2 in zemljevid if x1 == tocka):
            zahtevnost = vrednost_povezave((x1, x2), zemljevid) 
            if meja >= zahtevnost and x2 not in banned: 
                dosegljivetocke = dosegljivetocke | doseg((x2, meja - zahtevnost), zemljevid, banned | set(tocka)) 
