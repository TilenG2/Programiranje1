...
def mozen_skok(x, y, ovira):
    yOvire, x0Ovire, x1Ovire = ovira
    return (y - 2 <= yOvire <= y + 2 and y != yOvire and x0Ovire <= x + 1 <= x1Ovire
         or y - 1 <= yOvire <= y + 1 and x0Ovire <= x + 2 <= x1Ovire)

def mozne_ovire(x, y):
    return {ovira for ovira in ovire if mozen_skok(x, y, ovira)}

def obstaja_pot(ovira):
    yOvire, _, x1Ovire = ovira
    if x1Ovire >= 16: 
        return True
    moznosti = []
    for ovira in mozne_ovire(x1Ovire, yOvire): 
        moznosti.append(obstaja_pot(ovira))
    return any(moznosti)

def stevilo_poti(ovira):
    yOvire, _, x1Ovire = ovira
    if x1Ovire >= 16:
        return len(mozne_ovire(x1Ovire, yOvire)) + 1 
    moznosti = []
    for ovira in mozne_ovire(x1Ovire, yOvire): 
        moznosti.append(stevilo_poti(ovira))
    return sum(moznosti)
